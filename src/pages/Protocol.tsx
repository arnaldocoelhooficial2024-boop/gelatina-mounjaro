...
export function Protocol() {
  ...
  return (
    <div className="p-6 space-y-8 max-w-md mx-auto">
      ...
      <section className="bg-gradient-to-br from-brand-600 to-brand-800 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/20 relative overflow-hidden text-white">
      ...
      <div className="space-y-4">
        <h3 className="font-serif font-bold text-xl text-slate-800 dark:text-slate-200 px-2 mt-8 mb-4">Sua Jornada de 30 Dias</h3>
...
const DayCard: React.FC<{ 
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-[2rem] border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-brand-300 dark:border-brand-600 shadow-lg shadow-brand-500/10 dark:shadow-brand-900/20' : isCompleted ? 'border-emerald-200 dark:border-emerald-800/50 shadow-sm' : 'border-slate-100 dark:border-slate-700 shadow-sm hover:border-brand-200 dark:hover:border-brand-700'}`}>
      <button 
        onClick={onToggle}
        className="w-full flex items-center justify-between p-6 text-left focus:outline-none"
      >
        <div className="flex items-center gap-5">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-xl font-serif transition-colors relative ${isCompleted ? 'bg-emerald-500 text-white shadow-inner' : isExpanded ? 'bg-brand-600 dark:bg-brand-500 text-white shadow-inner' : 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-slate-600'}`}>
            {isCompleted ? <Check size={28} strokeWidth={3} /> : data.day}
          </div>
          <div>
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300 shrink-0 ${isExpanded ? 'bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}>
        ...
        {isExpanded && (
          ...
          >
            <div className="p-6 pt-0 space-y-6">
              ...
              <div className="flex gap-4 items-start bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
